...
export {
  htmlEncode,
  el,
  getElementsIncludingSelfByTextContent,
  appendHtmlStrToBody,
  appendHtml,
};
...
window.elExists = function elExists(id) {
...
function el(id) {
  ...
  checkThat({
    condition: rv != null,
    errMsgFn: function () {
      return "ERROR: No element exists having id '" + id + "'!!!";
    },
  });
  return rv;
}
...
window.getNearestAncestorWhoseIdContains = function (params) {
  ...
  while (true) {
    ...
    if (el.id.toLowerCase().includes(s)) {
      return el;
    }
  }
};

window.getSelfOrNearestAncestorWhoseClassNameIs = function (params) {
...
window.getNearestAncestorWhoseClassNameIs = function (params) {
  checkParams(params, ["el", "className"]);
  let el = params.el;
  if (el === document) {
    return null;
...
window.getNearestPrevEleWhoseClassNameIs = function (params) {
  checkParams(params, ["el", "className"]);
  let el = params.el;
  if (el === document) {
    return null;
...
window.getNearestDescendantWhoseClassNameAndAttributeIs = function (params) {
...
window.getNearestDescendantWhoseClassNameIs = function (params) {
  checkParams(params, ["el", "className"]);
  let el = params.el;
  let className = params.className;
  let children = Array.from(el.children);
...
window.getNearestSingleDescendantWhoseClassNameIs = function (params) {
  checkParams(params, ["el", "className"]);
  let el = params.el;
  let className = params.className;
  let children = Array.from(el.children);
...
window.tabEleForTabPaneThatContains = function (el) {
  ...
  let tabEle = getNearestDescendantWhoseClassNameAndAttributeIs({
    el: tabContentDivParent,
    className: "nav-link",
    attrName: "href",
    attrVal: "#" + tabPaneDiv.id,
  });
  return tabEle;
};

window.makeallancestorsvisible = function (el, nodesMadeVisible) {
...
function appendHtmlStrToBody(html, url, doc) {
  ...
  dbg &&
    console.log(
      "Created a docFragment with children.length=" +
  ...
  dbg &&
    console.log(
      uniqueMsg(
        "Before setInnerHTML(),docFragment.html:\n" + docFragment.innerHTML
      )
    );
  setInnerHTML(docFragment, html, url);
  dbg &&
    console.log(
      "After setInnerHTML(),docFragment.html with urls " +
        "made absolute:\n" +
        docFragment.innerHTML
    );
  dbg &&
    console.log(
      "After setInnerHTML(),docFragment.children.length=" +
        docFragment.children.length
    );
  dbg &&
    console.log(
      uniqueMsg(
        "Before appending doc fragment, " +
  ...
  dbg &&
    bigLog(
      new Error(),
      "Before appending doc fragment from url '" +
        url +
        "', doc.body.innerHTML:\n" +
        doc.body.innerHTML
    );
  ...
  dbg &&
    console.log(
      uniqueMsg(
        "After appending doc fragment from url '" +
          url +
          "', doc.body.children.length=" +
          doc.body.children.length
      )
  ...
  dbg &&
    bigLog(
      new Error(),
      "After appending doc fragment, doc.body.innerHTML:\n" + doc.body.innerHTML
    );
  /*
...
function setInnerHTML(elm, html, url) {
  ...
  dbg &&
    console.log(
      uniqueMsg(
        "setInnerHTML(): After making urls absolute w.r.t '" +
          url +
          "', elm.innerHTML:\n" +
          elm.innerHTML
      )
  ...
  dbg &&
    console.log("setInnerHTML(): At exit: elm.innerHTML:\n" + elm.innerHTML);
}
...
function transformUrlsOfEleToBeAbsolute(ele, url, dbg) {
  ...
  if (ele.src != null && ele.src != "") {
    ...
    let changedUrl = getFullUrlOfUrlXThatIsRelativeToUrlY(urlToChange, url);
...
function transformUrlOfScriptToBeAbsolute(ele, url) {
  ...
  if (ele.innerHTML.indexOf("/*begin relpath*/") == -1) {
    return;
  }
  ...
  scriptAreas.forEach((scriptArea, idx) => {
    if (
      scriptArea.type == "comment" &&
    ...
    if (
      ...
      let changedUrl = getFullUrlOfUrlXThatIsRelativeToUrlY(urlToChange, url);
      dbg && console.log("changedUrl='" + changedUrl + "'");
      scriptAreas[idx - 1].text =
        openingQuoteChar + changedUrl + closingQuoteChar;
...
window.insertAsFirstChild = function insertAsFirstChild(parent, eleToIns) {
...
window.getElsByClsNm = function getElsByClsNm(nm) {
 ...
 * @returns {HTMLElement[]}
 */
function getElementsIncludingSelfByTextContent(container, textContent) {
...
(function ($) {
  $.fn.closestPrior = function (selector) {
    return closestPrior(this, selector);
  };
})(jQuery);
